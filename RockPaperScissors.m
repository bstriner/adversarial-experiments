...
diffs = [a_diff; b_diff];
params = [a_theta; b_theta];
diff2 = sym('diff2',[6 6]);
%diff2 = cell(6,6);
for i = 1:6
   for j = 1:6
      diff2(i,j) = diff(diffs(i), params(j));
   end
end

paramst = [1;1;1;1;1;1];
diff3 = double(subs(diff2, params, paramst));


eigs = eig(eye(6) - lr*diff3);
disp(eigs);
disp(abs(eigs));
...
for epoch=1:nb_epoch
    ...
    a_t = double(subs(a, a_theta, a_theta_t));
    b_t = double(subs(b, b_theta, b_theta_t));
    ...
    for batch=1:nb_batches
        ...
        a_theta_next = double(subs(subs(a_theta_update,a_theta, a_theta_t), b_theta, b_theta_t));
        b_theta_next = double(subs(subs(b_theta_update,a_theta, a_theta_t), b_theta, b_theta_t));
        % Store next theta
